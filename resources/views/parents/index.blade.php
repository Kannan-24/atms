<x-app-layout>
    ...
    <x-slot name="title">
        {{ __('Parent List') }} - {{ config('app.name', 'ATMS') }}
    </x-slot>
                    ...
                    <table class="min-w-full text-left border-collapse table-auto">
                        <thead>
                            <tr class="text-sm text-gray-600 bg-indigo-100">
                                ...
                                <th class="px-6 py-4 border-b-2 border-gray-200">Phone Number</th>
                        ...
                        <tbody class="text-sm text-gray-700">
                            @foreach ($parents as $parent)
                                <tr class="border-b hover:bg-indigo-50">
                                    ...
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $parent->user->name }}</td>
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $parent->user->email }}</td>
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $parent->user->phone }}</td>
                                    <x-action-buttons model="parents" :id="$parent->id" />
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
